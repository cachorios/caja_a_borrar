...
class TipoPagoController extends Controller
{
	...
	public function createAction()
	{
		...
		if($form->isValid()) {
			...
			return $this->redirect($this->generateUrl('tipopago_show', array('id' => $entity->getId())));
		} else {
	 ...
	 * ponerDivisible
	 *
	 * Permite que solo un registro tenga actiovo el campo divisible
	 * a todos los distintos de $id le pone el false.
	 *
	 * @param $id
	 */
	private function ponerDivisible($id)
	{
		...
		$em->createQuery("
				update SistemaCajaBundle:TipoPago u
				set u.divisible = false
				where u.id<> :id ")
			->setParameter("id", $id)
			->execute();
	}
